...
Fixes & improvements:
- ✅ Pillow 10+ compatibility shim (ANTIALIAS/BILINEAR/BICUBIC).
- ✅ **Default output is 1080p** (`--max-height 1080`).
- ✅ Progress logging via `--logger` (bar/verbose/none) so you can see it's working.
- ✅ Safer resource handling; closes clips to reduce RAM usage.
- ✅ Optional `--selftest` creates tiny synthetic clips and runs the pipeline.

Usage examples (PowerShell / CMD):
  python scripts/assemble_video.py
  python scripts/assemble_video.py --logger bar --max-height 1080 --fps 30 --target-seconds 90 --max-clips 18
...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--logger", choices=["bar", "verbose", "none"], default="bar", help="Progress logger")
...
def write_video(selected, args: argparse.Namespace):
    print(f"Concatenating {len(selected)} clips (~{sum([c.duration for c in selected]):.1f}s) …")
    t0 = time.time()
    video = concatenate_videoclips(selected, method="compose")
    print(f"Concatenated in {time.time()-t0:.1f}s. Writing to {args.output} …")
